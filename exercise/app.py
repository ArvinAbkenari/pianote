...
if not reference_files:
    ...
    if reference_choice and user_file:
        ...
        # --- Note Extraction (Do Re Mi) ---
        NOTE_NAMES_SOLFEGE = [
            'do', 'do#', 're', 're#', 'mi', 'fa',
            'fa#', 'sol', 'sol#', 'la', 'la#', 'si'
        ]

        def chroma_to_notes_solfege(
            chroma, sr, hop_length, threshold=0.3, min_note_length=3
        ):
            notes = []
            prev_idx = None
            count = 0
        ...
        ref_notes = chroma_to_notes_solfege(
            ref_chroma, ref_sr, chroma_hop,
            threshold=0.3, min_note_length=3
        )
        user_notes = chroma_to_notes_solfege(
            user_chroma, user_sr, chroma_hop,
            threshold=0.3, min_note_length=3
        )
        ...
        st.caption(
            "Analysis uses high-resolution onset detection and DTW on "
            "chroma features for super-accurate timing and pitch comparison. "
            "For best results, use clean solo piano recordings of "
            "the same piece."
        )